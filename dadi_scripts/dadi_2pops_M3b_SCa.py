...
# # Secondary contact model, asymmetrical migration (SCa)


pts_1 = [200,220,240]

def SC(params, ns, pts):
	nuP, nuR, T1, T2, mPR, mRP = params
	"""
	...
	mRP:  migration from pungens into rigid
	...
	phi = dadi.Integration.two_pops(phi,xx,T2,nu1=nuP, nu2=nuR, m12=mPR, m21=mRP)
...
upper_bound = [100, 100, 10, 10,50, 50] 
lower_bound = [1e-5,1e-5,1e-6,1e-6, 0.1,0.1]
p0 = array([0.05,0.05,0.15,0.15,10,10])
...
matplotlib.pyplot.savefig("M3b_SCasym.pdf")
