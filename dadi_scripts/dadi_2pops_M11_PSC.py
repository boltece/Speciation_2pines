...
# ## Instantaneous size change, no migration (PSC)


pts_1 = [200,220,240]

def size(params, ns, pts):
	nuP1, nuR1, T1, nuP2, nuR2, T2 = params
	"""
	2-populations, instantaneous size change with no gene flow.
	...
	phi = dadi.Integration.two_pops(phi,xx,T1,nu1=nuP1, nu2=nuR1)
	phi = dadi.Integration.two_pops(phi,xx,T2,nu1=nuP2, nu2=nuR2)
...
upper_bound = [100, 100, 20, 100, 100, 20] 
lower_bound = [1e-4,1e-4,1e-3,1e-4, 1e-4, 1e-3]
p0 = array([0.5,0.5,0.01,0.5,0.5, 0.01])
...
matplotlib.pyplot.savefig("M11_sizeChange.pdf")
